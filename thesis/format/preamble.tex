...
% NOTE: load hyperref late (after packages that may create page anchors)
% We'll load it after `lastpage` further down to avoid duplicate page anchor definitions.
...
% siunitx disables \qty when the `physics` package defines it.
% Restore siunitx's \qty to match \SI behavior (recommended by siunitx).
...
% Load hyperref after packages that define page anchors (like lastpage).
\usepackage{hyperref}
